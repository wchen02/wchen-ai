...
**1. Gather corpus signal.** Read a representative set of writings from `content/writing/*.mdx` (and optionally locale-specific writing) — prefer recent and `featured: true`. From each piece: `theme`, `tags`, and one or two concrete convictions or reframes from the body. Then read projects from `content/projects/*.mdx`, prefer `featured` and `status: active` or `in-progress`. From each: `motivation`, `problemAddressed`, `learnings`, `type`, and recurring problem areas. Synthesize: current themes, problems the site owner cares about, principles that repeat, expertise that shows up in both writing and projects.
