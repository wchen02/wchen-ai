...
I tightened the skill and content-schemas so the content model is unambiguous. Site copy is locale-scoped JSON only under `content/locales/<locale>/site/*.json`; there is no shared source for home, about, or UI. For writing and projects, each locale uses *either* that locale’s folder *or* the shared folder — no merging. Shared `content/writing` and `content/projects` are the canonical default; the content-translation skill produces locale copies after I finish a shared entry. I also documented that `content/site/newsletter-state.json` is script-managed and should not be edited for copy.

The voice checklist and content-schemas now reference Zod in `src/lib/schemas.ts` and the validate scripts (`validate-links`, `validate-theme-descriptors`, `validate-metadata`) so the agent knows invalid content fails the build. Clearer boundaries mean the agent stops guessing where files go and what not to touch.
