...
import { type Project, type ProjectType } from "@/lib/schemas";
import { getUiContent } from "@/lib/site-content";

const PROJECT_TYPES: ProjectType[] = ["app", "agent", "experiment", "skill", "library", "tool"];
...
export default function ProjectsFilter({ projects }: { projects: Project[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return projects.filter((p) => p.type.includes(typeFilter));
  }, [projects, typeFilter]);
    ...
    <div className="space-y-6">
      {showFilter && (
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-sm font-medium text-gray-600 dark:text-gray-400">{uiContent.projects.filterByTypeLabel}</span>
          <button
            type="button"
          ...
          {PROJECT_TYPES.map((value) => (
            ...
            >
              {uiContent.projects.typeLabels[value]}
            </button>
          ))}
        </div>
      )}
